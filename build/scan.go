...
// so every Claude model can be A/B tested without a rebuild. Start on Sonnet.
const defaultScanModel = "claude-sonnet-4-6"
...
// scanWithClaude calls the Anthropic API and parses the nutrition JSON out of the reply.
func scanWithClaude(content []map[string]any) (*ScanResult, error) {
